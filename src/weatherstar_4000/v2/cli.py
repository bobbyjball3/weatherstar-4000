"""Command-line entrypoint for the WeatherStar 4000 v2 engine.

Usage::

    weatherstar4000-v2 [--config PATH] [--sequence NAME] [--lat F] [--lon F]
                       [--log-level LEVEL] [--log-file PATH] [--no-console]
                       [--validate] [--frames N]
    weatherstar4000-v2 generate-config [--sequence NAME] [-o PATH]

Sequence precedence is CLI flag > WEATHERSTAR_SEQUENCE envvar > config
``sequence`` key.  A config file (default ~/.config/weatherstar4000/config.toml,
...
from weatherstar_4000.v2 import engine
from weatherstar_4000.v2.config_file import (
    ENV_SEQUENCE,
    AppConfig,
    discover_config_path,
)
from weatherstar_4000.v2.errors import ConfigError, WeatherStarError
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="weatherstar4000-v2")
...
def _generate_config(args: argparse.Namespace) -> int:
    from weatherstar_4000.v2 import skeleton
...
def _run(args: argparse.Namespace) -> int:
    ...
    from weatherstar_4000.v2.logging_setup import setup_logging
    ...
    pygame.display.set_caption("WeatherStar 4000 v2")
