...
:class:`~weatherstar_4000.context.AppContext`: which colors resolve, which
...
(``~/.config/weatherstar4000/themes/``), then the built-in themes shipped with
...
log = get_logger("weatherstar4000.themes")
...
def xdg_themes_dir() -> Path:
    ...
    return root / "weatherstar4000" / "themes"
...
def _theme_from_file(path: Path) -> Theme | None:
    ...
    try:
        title = str(data.get("title") or "WeatherStar 4000")
