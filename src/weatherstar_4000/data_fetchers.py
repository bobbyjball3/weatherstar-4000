...
class WeatherStarDataFetchers:
    ...
    def fetch_radar_image(self):
        ...
        try:
            ...
            self.logger.main_logger.info(
                f"Fetching regional radar for lat:{self.ws.lat}, lon:{self.ws.lon}"
            )
                ...
                for url in radar_urls:
                    try:
                        response = requests.get(
                            url, timeout=3, headers={"User-Agent": "WeatherStar4000"}
                        )
                        ...
                        if response.status_code == 200 and len(response.content) > 1000:
                            ...
                            crop_box = self._calculate_crop_area(
                                self.ws.lat, self.ws.lon, pil_img.size
                            )
            ...
            if self.ws.radar_frames:
                ...
                self.logger.main_logger.info(
                    f"Successfully loaded {len(self.ws.radar_frames)} radar frames"
                )
                return True
    ...
    def update_scroll_text(self):
        ...
        try:
            ...
            if current:
                ...
                if temp_c is not None:
                    ...
                    if forecast and forecast.get("periods"):
                        ...
                        weather_str += (
                            f"  •  {period.get('name', '')}: {period.get('shortForecast', '')}"
                        )
            ...
            self.ws.scroller.current_text = (
                f"WeatherStar 4000+ - {self.ws.location.get('city', 'Loading')}"
            )
