...
def draw_severe_weather_alert(ws_instance, dt):
    ...
    if alerts:
        ...
        try:
            ...
            text = font.render(
                "NO ACTIVE WEATHER ALERTS", True, ws_instance.current_theme.get_color("white")
            )
            text_rect = text.get_rect(center=ws_instance.screen.get_rect().center)
            ws_instance.screen.blit(text, text_rect)

            font_small = pygame.font.Font(None, 32)
            subtext = font_small.render(
                "All weather conditions normal", True, ws_instance.current_theme.get_color("white")
            )
            subtext_rect = subtext.get_rect(
                center=(
                    ws_instance.screen.get_rect().centerx,
