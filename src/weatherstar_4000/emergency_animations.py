...
class EmergencyAlertAnimator:
    ...
    def draw_scrolling_text(
        ...
        bg_color: tuple[int, int, int] = (139, 0, 0),
    ) -> None:
        ...
        # Draw scrolling text
        surface.blit(
            text_surface, (scroll_x, rect.y + (rect.height - text_surface.get_height()) // 2)
        )
        ...
        if scroll_x < rect.x:
            surface.blit(
                text_surface,
                (
                    scroll_x + text_width + rect.width,
                    rect.y + (rect.height - text_surface.get_height()) // 2,
                ),
            )

    def draw_alert_header(
        self,
        surface: pygame.Surface,
        rect: pygame.Rect,
        alert_title: str,
...
class SevereWeatherDisplay:
    ...
    def draw_full_alert_screen(
        self, surface: pygame.Surface, dt: float, theme_colors: dict
    ) -> None:
        ...
        self.animator.draw_alert_header(surface, header_rect, alert_type, self.alert_font_large)
        ...
        self.animator.draw_scrolling_text(surface, scroll_rect, message, self.alert_font_medium)
