...
class ExtendedForecastScreen(Screen):
    ...
    def compose(self, surface: pygame.Surface, ctx: Any, dt: float) -> None:
        ...
        for i in range(0, min(len(periods), 6), 2):
            ...
            original_icon = self.icon_surface(ctx, self.icon_name(str(day_period.get("icon", ""))))
            ...
            if lo_temp is not None:
                ...
                lo_surf = self.font(ctx, "normal").render(f"{lo_temp}\N{DEGREE SIGN}", True, white)
            ...
            if hi_temp is not None:
                ...
                hi_surf = self.font(ctx, "normal").render(f"{hi_temp}\N{DEGREE SIGN}", True, white)
