...
def fetch_google_news(location: str) -> list[tuple[str, str]]:
    ...
    try:
        ...
        # Fetch RSS feed
        headers = {"User-Agent": "WeatherStar4000/1.0 (Python RSS Reader)"}
                        ...
                        for item in root.findall(".//item")[:2]:
                            ...
                            if title_elem is not None and link_elem is not None:
                                ...
                                if not any(
                                    ...
                                    for prefix in ["Breaking:", "Alert:", "Local:"]
                                ):
                                    title = f"Local: {title}"
...
def fetch_weather_alerts(lat: float, lon: float) -> list[tuple[str, str]]:
    ...
    try:
        ...
        headers = {"User-Agent": "WeatherStar4000/1.0", "Accept": "application/json"}
        ...
        if response.status_code == 200:
            ...
            for alert in features[:3]:  # Get up to 3 alerts
                ...
                if event:
                    ...
                    url = (
                        f"https://www.weather.gov/alerts/{alert_id}"
                        if alert_id
