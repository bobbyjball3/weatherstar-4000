...
class AnimatedIcon:
    ...
    def load_gif(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded animated icon: {os.path.basename(self.filepath)} ({len(self.frames)} frames)"
            )
    ...
    def get_scaled_frame(self, width: int, height: int) -> pygame.Surface | None:
...
class AnimatedIconManager:
    ...
    def load_all_icons(self):
        ...
        logger.info(
            f"Loaded {len(self.animated_icons)} animated icons and {len(self.static_icons)} static icons"
        )

    def get_icon(
        self, icon_name: str, width: int | None = None, height: int | None = None
    ) -> pygame.Surface | None:
