...
class WeatherAPIBase:
    ...
    def _is_cache_valid(self, key: str, max_age: int | None = None) -> bool:
    ...
    def _get_cached(self, key: str, max_age: int | None = None) -> Any | None:
    ...
    def _fetch_json(self, url: str, params: dict = None, timeout: int = 10) -> dict | None:
...
class NOAAWeatherAPI(WeatherAPIBase):
    ...
    def get_current_conditions(self, lat: float, lon: float) -> dict | None:
...
class OpenMeteoAPI(WeatherAPIBase):
    ...
    def get_current_weather(self, lat: float, lon: float, units: str = "imperial") -> dict | None:
    ...
    def get_forecast(
        self, lat: float, lon: float, days: int = 7, units: str = "imperial"
    ) -> dict | None:
...
class UnifiedWeatherAPI:
    ...
    def _is_us_location(self, lat: float, lon: float) -> bool:
        ...
        us_bounds = {"lat_min": 24.0, "lat_max": 49.0, "lon_min": -125.0, "lon_max": -66.0}
    ...
    def get_weather(self, lat: float, lon: float, prefer_international: bool = False) -> dict:
        ...
        if not prefer_international and self._is_us_location(lat, lon):
            ...
            if current or forecast:
                return {"source": "NOAA", "current": current, "forecast": forecast}
        ...
        return {"source": "Open Meteo", "current": current, "forecast": forecast}
